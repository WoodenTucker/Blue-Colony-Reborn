...
/datum/controller/subsystem/elections/proc/postelection_news_article()
	...
	news_network.SubmitArticle(message, pick(electoral_assistants), "Kiel Standard", null, null, "", "[current_president.name] Wins [get_month_from_num(get_game_month())] [get_game_year()] Election with [current_president.ckeys_voted.len] Votes Out of [last_election_votes]")
